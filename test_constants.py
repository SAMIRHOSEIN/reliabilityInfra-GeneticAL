...
K_TOURNAMENT=3 #5                            # benchmark = 3

ELE_GA_CROSSOVER_TYPE = "uniform"            # benchmark = crossover_type="single_point"     Rationale: cause  genes are continuous β-thresholds; Randomly selects each gene from one of the parents
...
if ELE_GA_CROSSOVER_TYPE == "uniform" or ELE_GA_CROSSOVER_TYPE == "scattered":
    ...
    """
    ELE_GA_CROSSOVER_PROBABILITY = 0.5       # benchmark =(None means crossover is applied to every mating pair)  - This parameter is used only in 'uniform' crossover or scattered crossover
...
# # region: constants for pygad_reliability.py ==================================
# ELE_GA_SEED_FOR_PyGAD = 0
# ELE_GA_POP = 256 #512#128 #80                        # Population size - (Population * Genes = 128*256) ~ (PPO frames/horizon = 5*32*1024/5 = 32768)
# ELE_GA_GENS = 512# 1500 #256 #256 #200               # Number of generations - (Population * Genes = 128*256) ~ (PPO frames/horizon = 5*32*1024/5 = 32768)
...
#             - If it's greater than or equal to 0.7, the gene is kept as-is.
#     """
#     ELE_GA_CROSSOVER_PROBABILITY = 0.5       #default = None(None means crossover is applied to every mating pair)  - This parameter is used only in 'uniform' crossover or scattered crossover
